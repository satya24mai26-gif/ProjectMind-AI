...
from "@/components/ui/ToolbarButton";
...
const loadLocalChat = (): ChatTreeItem[] => {
  ...
  // Safe Zustand state triggers (Only select primitive values or immutable functions)
  ...
  const syncProjectHistory = async (id: number) => {
    ...
    try {
      ...
      if (activeParentId !== null) {
        const newestLeaf = getLatestBranchLeaf(activeParentId);
  ...
  const activeDisplayMessages = useMemo((): ChatTreeItem[] => {
    ...
    if (activeParentId === null) {
      const roots = chatMessagesArray
        .filter(m => m.role === "user" && m.parent_id === null);
    ...
    roots.forEach(root => {
        ...
        const assistant = chatMessagesArray.find(
            m =>
                m.role === "assistant" &&
                m.parent_id === root.id
        );
    ...
    while (traversalId !== null && iterations < maxIterations) {
      ...
      if (userPrompt) {
        ...
        const aiResponse = chatMessagesArray.find(
          m => m.role === "assistant" && m.parent_id === userPrompt.id
        );
        pairs.unshift({
          user: userPrompt,
          ai: aiResponse
    ...
    return ordered;
  }, [chatMessagesArray, activeParentId]);
  ...
  const handleSend = async (options?: { overridePrompt?: string; editMessageId?: number; isRegenerate?: boolean; clearAll?: boolean }) => {
    ...
    try {
      const context = { node: { id: projectId }, outgoingRelations: [], incomingRelations: [] }; // Mock schema context anchor fallback
      ...
      // immediately open this branch
      
      setActiveParentId(tempUserId);

      await askProjectAIAdvanced(projectId, targetPrompt, options?.editMessageId, options?.isRegenerate, options?.clearAll, null, activeParentId);
      ...
      const currentParent = activeParentId;

      const latestUser = [...freshHistory]
        .reverse()
        .find(
          (m: any) =>
            m.role === "user" &&
            Number(m.parent_id) === currentParent
        );

      if (latestUser) {
        setActiveParentId(Number(latestUser.id));
      }

      if (!options?.editMessageId && !options?.isRegenerate && !options?.clearAll && activeParentId === null) {
        const freshHistory = await fetchProjectChatHistory(projectId);
  ...
  // --- RECURSIVE SIDEBAR TREE NODE GENERATOR ---
  const renderSidebarTreeNodes = (parentId: number | null, depth = 0) => {
    const userNodes = chatMessagesArray.filter(
        ...
        <div key={uNode.id} className="flex flex-col w-full">
          <button
            onClick={() => {
              setIsNewChat(false);
              const newestLeaf =
                getLatestBranchLeaf(Number(uNode.id));
          ...
          >
            <span
  className={`mr-1.5 font-mono text-[9px] ${
    isCurrentLeaf
      ? "text-cyan-400"
      : isSelectedBranch
      ? "text-indigo-500"
...
>
  ...
  // 🛡️ CRITICAL RULES OF HOOKS ENFORCEMENT: Shshifted safely beneath all hook initializations
  if (!projectId) return null;

  return (
    <div
      className={`flex flex-col bg-white border border-slate-200 shadow-xl transition-all duration-300 ease-in-out ${
        isFullScreen
          ? "fixed top-4 left-4 right-4 bottom-4 z-50 rounded-2xl h-auto bg-white/95 backdrop-blur-md"
          : "rounded-xl w-full h-145"
      }`}
    >
      <div className="flex items-center gap-2 p-2">
        <div className="h-2 w-2 rounded-full bg-indigo-500 animate-pulse" />
        <h3 className="text-xs font-bold text-slate-950 uppercase tracking-wider">
          Project Copilot Workspace
        </h3>
      </div>
      ...
      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/90 px-4 py-3 shrink-0">
        <div className="relative group">
          <ToolbarButton
            position="top-left"
            icon="၊၊||၊"
            tooltip="Open Branch Navigator"
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="px-2 py-1 bg-white border border-slate-200 rounded-md text-xs font-bold text-slate-700 hover:bg-slate-50 shadow-sm"
          />
        </div>

        <div className="flex items-center gap-1.5">
          <ToolbarButton
            icon={isFullScreen ? "➕ New Branch" : "➕"}
          ...
          {chatMessagesArray.length > 0 && (
            <ToolbarButton
              icon={isFullScreen ? "🗑 Reset Log" : "🗑"}
              tooltip="Delete Entire Chat History"
              onClick={handleWipeFullChat}
              className="text-[11px] text-red-500 border border-slate-200 text-slate-700 hover:bg-red-50 px-2 py-1 rounded-md font-semibold transition-colors"
            />
          )}
          <ToolbarButton
            icon={isFullScreen ? "🗗 Compact" : "🗖 Fullscreen"}
            tooltip={isFullScreen ? "Exit Fullscreen" : "Enter Fullscreen"}
            onClick={() => setIsFullScreen(!isFullScreen)}
            className="text-[11px]  bg-slate-900  text-white px-2.5 py-1  rounded-md font-bold    hover:bg-slate-800"
          />
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden divide-x divide-slate-100 bg-white">
        {/* Thread sidebar explorer */}
        <aside
          className={`bg-slate-50/30 p-2 space-y-1 overflow-y-auto flex flex-col shrink-0 transition-all duration-300 ${
            isSidebarOpen
              ? "w-52 opacity-100"
              : "w-0 opacity-0 pointer-events-none !p-0 border-none"
          }`}
        >
          <div className="text-[10px] font-bold text-slate-400 px-2 py-1 uppercase tracking-wider mb-1 select-none border-b border-slate-100">
            Discussion Threads
          </div>
          <div className="flex-1 space-y-1">
            {renderSidebarTreeNodes(null)}
            {chatMessagesArray.filter((m) => m.role === "user").length ===
              0 && (
              <div className="text-xs text-slate-400 italic px-2 py-4">
                No active branches.
              </div>
            )}
          </div>
        ...
        {/* Messaging Thread Stream Panel */}
        <div className="flex-1 flex flex-col overflow-hidden bg-white">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {/* Conversation Thread Scroller list */}
            {isNewChat ? (
              ...
              <div className="flex flex-col h-full items-center justify-center text-slate-400 text-center py-12 space-y-2 select-none">
                ...
                <p className="text-xs font-semibold max-w-xs text-slate-500">
                  Query the project graph context or spawn nested discussion
                  branches from the left thread panel mapping tree.
                </p>
              </div>
            ) : (
              activeDisplayMessages.map((msg, idx) => {
                ...
                return (
                  <div
                    key={msg.id || idx}
                    className={`group relative flex flex-col max-w-[85%] rounded-xl px-4 py-3 text-sm shadow-sm border transition-all ${
                      isUser
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-[-14px] opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex items-center bg-white border border-slate-200 rounded-md shadow-md px-2 py-0.5 gap-2 z-10 ${
                    ...
                    {/* Content Router Parsing Blocks */}
                    {isEditing ? (
                      <div className="space-y-2 py-1 min-w-[240px]">
                        <textarea
                          value={editValue}
                          onChange={(e) => setEditValue(e.target.value)}
                          className="w-full bg-slate-800 text-white border border-slate-700 rounded p-2 text-xs focus:outline-none resize-none font-sans"
                          rows={2}
                        />
                        <div className="flex justify-end gap-1.5">
                          <button
                            onClick={() => setEditingMessageId(null)}
                            className="px-2 py-0.5 text-[10px] bg-slate-700 text-slate-200 rounded font-medium"
                          >
                            Cancel
                          </button>
                          <button
                            onClick={() =>
                              handleSend({
                                overridePrompt: editValue.trim(),
                                editMessageId: msg.id,
                              })
                            }
                            className="px-2 py-0.5 text-[10px] bg-cyan-600 text-white rounded font-medium"
                          >
                            Save
                          </button>
                        </div>
            ...
            {errorMsg && (
              <div className="max-w-[85%] rounded-lg bg-red-50 border border-red-100 p-2 text-xs text-red-700 font-medium">
                {errorMsg}
              </div>
            )}
          ...
          <div className="border-t border-slate-100 p-3 bg-white shrink-0">
            ...
            >
              <input
                ...
                className="h-10 flex-1 rounded-lg border border-slate-200 bg-slate-50/50 px-3.5 text-sm outline-none transition-all placeholder:text-slate-400 focus:border-slate-400 focus:bg-white"
                placeholder={
                  activeParentId
                    ? "Replying inside this sub-thread branch..."
                    : "Query project knowledge graph maps..."
                }
              />
              <button
                type="submit"
                disabled={!message.trim() || isLoading || isHistoryLoading}
                className="h-10 rounded-lg bg-slate-900 px-5 text-sm font-semibold text-white hover:bg-slate-800 transition-all shadow-sm"
              >
                Send
              </button>
            </form>
      ...
      <div>
        <button
          onClick={async () => {
            const result = await analyzeGraphRelationships(projectId);

            setGraphRelationships(result.missing_relationships || []);
          }}
          className="
        w-full
        bg-indigo-600
            ...
            onClick={async () => {
              ...
              for (const relationship of graphRelationships) {
                const sourceNode = nodes.find(
                  (n) => n.data.title === relationship.source
                );

                const targetNode = nodes.find(
                  (n) => n.data.title === relationship.target
                );
